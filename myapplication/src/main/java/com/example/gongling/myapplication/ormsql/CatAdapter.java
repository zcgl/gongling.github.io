...
public class CatAdapter extends RecyclerView.Adapter<CatAdapter.CatHolder> {
    ...
    int selectedpos=0;
    public CatAdapter(Activity context,List<ProductCategory> productCategoryList) {
    ...
    @Override
    public CatAdapter.CatHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        CatHolder catHolder=new CatHolder(LayoutInflater.from(
                context).inflate(R.layout.product_cat_item, parent,
                false),mItemClickListener);
        return catHolder;
    }

    @Override
    public void onBindViewHolder(CatAdapter.CatHolder holder, int position) {

        holder.catName.setText(productCategoryList.get(position).getName());
        holder.catNum.setText("0");
        if(selectedpos==position){
            holder.catLine.setVisibility(View.VISIBLE);
            holder.catLyout.getBackground().setAlpha(255);
        }else{
            holder.catLine.setVisibility(View.GONE);
            holder.catLyout.getBackground().setAlpha(0);
        }
    ...
    class CatHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public CatHolder(View view,MyItemClickListener mListener) {
            ...
            catNum = (TextView) view.findViewById(R.id.cat_num);
